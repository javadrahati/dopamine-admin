...
const Modal = ({
	...
	const handleSearch = () => {
		setLoading(true)
		Axios.get(`food/search?q=${searchKeyword}`)
			.then(res => {
				setLoading(false)
				setList(res.data)
			})
	...
	return (
		<CModal alignment={"top"} onClose={onClose} size="lg"  {...restProps} >
			<CModalHeader>افزودن غذا</CModalHeader>
			<CModalBody>
				<CRow>
					<CFormFloating>
					...
					{list.map(u => (
						<CListGroupItem key={u.id}
							...
							<CRow xs={{cols:'auto'}} className="align-items-center">
								<CCol>
									<CImage src={u.image} align="center" height={40} rounded className="bg-secondary"/>
								</CCol>
								<CCol>
