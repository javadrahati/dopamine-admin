...
const PostTrainerModal = ({ item=undefined, onClose=null, onItemUpdated = null, ...restProps }) => {
	const [formData, setFormData] = useState({nameFa: "", nameEn: "", bioFa: "", bioEn: "", introFa: "", introEn: "", avatar: ""})
	...
	const handleSubmitForm = () => {
		setLoading(true)
		const url = 'trainer' + (item && item.id ? ("/" + item.id) : "")
	...
	return (
		<CModal alignment={"center"}  {...restProps} onClose={onClose} size="xl" >
			<CModalHeader closeButton={true}>
				{item ? "ویرایش اطلاعات مربی" : "ثبت مربی جدید"}
			</CModalHeader>
			<CModalBody>
				<CForm>
			...
			<CModalFooter>
				{ onClose != null && <CButton variant="ghost" color="dark" onClick={onClose}>بستن</CButton>}
...
export default PostTrainerModal
